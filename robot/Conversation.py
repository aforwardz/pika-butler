...
class TTSHorn(object):
    ...
    def speak(self, text, way="HTTP"):
        """
        喇叭支持MQTT、TCP和HTTP
        """
        if way not in ("MQTT", "TCP", "HTTP"):
            logger.critical(f"喇叭播报方式错误，未知方式：{way}", stack_info=True)

        url = config.get("HORN_URL", "")
        if way == "HTTP":
            json_data = {"ctrl_cmd": {"play_voice_cmd": text}}
            resp = requests.post(url, json=json_data)
            if resp.status_code == 200:
                logger.info(f"喇叭播报成功")


class SimpleConversation(object):
    """
    语音输出基于可自动TTS的喇叭，只需通过api发送指令及语音文本即可
    """
    def __init__(self, profiling=False):
        self.brain, self.asr, self.ai, self.tts, self.nlu = None, None, None, None, None
        self.reInit()
